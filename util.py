...
def get_request(url):
    ...
    print("Successfully got request")
    return r
...
def read_request(request):
    ...
    '''
    try:
        print('Encoding is:', request.encoding)
        if request.encoding != 'ISO-8859-1':
            request.encoding = 'ISO-8859-1'
            #print('    Changed encoding to:', request.encoding)
...
def is_absolute_url(url):
    ...
    '''
    if url == "":
        print("This is an empty URL")
        return False

    print("This is an absolute URL: ", urllib.parse.urlparse(url).netloc != "")
...
def convert_if_relative_url(current_url, new_url):
    ...
    Examples:
        convert_if_relative_url("http://steven-universe.fandom.com", "/wiki/Gem_Glow") yields
            'http://steven-universe.fandom.com/wiki/Gem_Glow'
    '''
...
WIKI = ("https://steven-universe.fandom.com/wiki/")
LEN_WIKI = len(WIKI)

WIKI_HTTP = ("http://steven-universe.fandom.com/wiki/")
LEN_WIKI_HTTP = len(WIKI_HTTP)
...
def is_url_ok_to_follow(url, limiting_domain):
    ...
    Examples:
        is_url_ok_to_follow(
            "steven-universe.fandom.com/wiki/Adventures_in_Light_Distortion/Transcript",
            "steven-universe.fandom.com/wiki")
            yields True

        is_url_ok_to_follow(
            "harrypotter.fandom.com/wiki/Main_Page",
            "steven-universe.fandom.com/wiki")
            yields False
    '''
    ...
    if url[:LEN_WIKI] == WIKI or url[:LEN_WIKI_HTTP] == WIKI_HTTP:
        return False
